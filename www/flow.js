...
(function () {
  ...
  function bindEvents() {
    ...
    // Capture Modal 事件
    document.getElementById('captureModalClose').addEventListener('click', closeCaptureModal);
    document.getElementById('captureCancelBtn').addEventListener('click', closeCaptureModal);
    document.getElementById('captureSaveBtn').addEventListener('click', saveFromCapture);
    captureUrlInput.addEventListener('input', handleCaptureUrlInput);
    
    // Capture Modal 文件处理
    captureDropZone.addEventListener('click', () => document.getElementById('captureFileInput').click());
    document.getElementById('captureFileInput').addEventListener('change', (e) => {
      if (e.target.files[0]) {
        capturePendingFile = e.target.files[0];
        captureStatus.textContent = `✓ 已选择: ${capturePendingFile.name}`;
        if (capturePendingFile.name.toLowerCase().endsWith('.epub')) {
          captureModeSelect.value = 'book';
        } else if (capturePendingFile.type.startsWith('audio/')) {
          captureModeSelect.value = 'audio';
        }
      }
    });

    // 气泡提示事件
    const clipboardBubble = document.getElementById('clipboardBubble');
    const clipboardBubbleAddBtn = document.getElementById('clipboardBubbleAddBtn');
    const clipboardBubbleCloseBtn = document.getElementById('clipboardBubbleCloseBtn');
    let currentBubbleUrl = '';

    clipboardBubbleAddBtn.addEventListener('click', () => {
      clipboardBubble.style.display = 'none';
      openCaptureModal(currentBubbleUrl);
    });

    clipboardBubbleCloseBtn.addEventListener('click', () => {
      clipboardBubble.classList.add('hiding');
      setTimeout(() => {
        clipboardBubble.style.display = 'none';
        clipboardBubble.classList.remove('hiding');
      }, 300);
    });

    function showClipboardBubble(url) {
      currentBubbleUrl = url;
      document.getElementById('clipboardBubbleText').textContent = `检测到链接: ${url.substring(0, 20)}...`;
      clipboardBubble.style.display = 'flex';
      
      // 5秒后自动隐藏
      setTimeout(() => {
        if (clipboardBubble.style.display === 'flex') {
          clipboardBubbleCloseBtn.click();
        }
      }, 5000);
    }
  ...
  let capturePendingFile = null;
  let captureMetadataTimer = null;

  function openCaptureModal(initialUrl = '') {
    resetCaptureForm();
    captureModal.classList.add('show');
    
    if (initialUrl) {
      captureUrlInput.value = initialUrl;
      handleCaptureUrlInput();
    }
  }
  ...
  function resetCaptureForm() {
    ...
    captureTitleInput.value = '';
    captureNoteInput.value = '';
    captureModeSelect.value = 'video';
    captureCategorySelect.value = 'read_later';
    captureImagePreview.style.display = 'none';
    captureImagePreview.querySelector('img').src = '';
    captureDropZone.style.display = 'block';
    captureStatus.textContent = '';
    capturePendingFile = null;
  }

  async function handleCaptureUrlInput() {
    const url = captureUrlInput.value.trim();
    if (!url || !url.startsWith('http')) return;

    clearTimeout(captureMetadataTimer);
    captureMetadataTimer = setTimeout(async () => {
      // 自动检测模式
      if (url.includes('youtube.com') || url.includes('youtu.be') || url.includes('bilibili.com')) {
        ...
        captureModeSelect.value = 'paper';
      } else {
        captureModeSelect.value = 'web';
      }

      captureStatus.textContent = '正在获取信息...';
      try {
        const metadata = await fetchMetadata(url);
        console.log('Capture抓取结果:', metadata);
        if (metadata.title) captureTitleInput.value = metadata.title;
        if (metadata.image) {
          captureImagePreview.style.display = 'block';
          captureImagePreview.querySelector('img').src = metadata.image;
          captureDropZone.style.display = 'none';
        } else {
          // 如果没抓到图，显示默认占位图或者保持显示拖拽区
          captureImagePreview.style.display = 'none';
          captureDropZone.style.display = 'block';
        }
        captureStatus.textContent = '✓ 获取成功';
      } catch (e) {
        captureStatus.textContent = '获取失败';
      }
    }, 500);
  }
  ...
  async function saveFromCapture() {
    const url = captureUrlInput.value.trim();
    const title = captureTitleInput.value.trim() || url || '未命名';
    
    if (!url && !capturePendingFile) {
      alert('请输入链接或上传文件');
      return;
    }
    ...
    const newItem = {
      ...
      url: url,
      title: title,
      category: captureCategorySelect.value,
      note: captureNoteInput.value.trim(),
      image: captureImagePreview.querySelector('img').src || '',
      platform: getPlatformFromMode(captureModeSelect.value, url),
      createdAt: new Date().toISOString(),
      pinned: false
    };

    if (capturePendingFile) {
      newItem.fileName = capturePendingFile.name;
      newItem.fileSize = capturePendingFile.size;
      newItem.fileData = await fileToBase64(capturePendingFile);
      if (captureModeSelect.value === 'book') {
        newItem.hasEpubFile = true;
      } else if (captureModeSelect.value === 'audio') {
  ...
  async function fetchMetadata(url) {
    ...
    if (url.includes('twitter.com') || url.includes('x.com')) {
      try {
        ...
        if (statusId) {
          ...
          if (res.ok) {
            ...
            if (data.tweet) {
              ...
              console.log('通过 fxtwitter API 成功获取元数据');
              return {
        ...
        console.warn('fxtwitter API 抓取失败:', e);
      }
    }

    // 2. 尝试使用 Netlify Function (云端抓取，逻辑已同步更新为支持 fxtwitter API)
    try {
      const cloudUrl = `https://info-flow.netlify.app/.netlify/functions/fetch-metadata?url=${encodeURIComponent(url)}`;
      const cloudRes = await fetch(cloudUrl);
      if (cloudRes.ok) {
        const data = await cloudRes.json();
        if (data && data.title && data.title !== 'X / ?') { // 过滤掉无效结果
          console.log('通过 Netlify Cloud 成功获取元数据');
          return data;
        }
      }
    } catch (e) {
      console.warn('Netlify Cloud 抓取失败:', e);
    }
    ...
    // 4. Twitter/X oEmbed 接口兜底
    let twitterTitle = '';
    let twitterImageFallback = '';
    if (url.includes('twitter.com') || url.includes('x.com')) {
      try {
        ...
        if (res.ok) {
          ...
          if (data.thumbnail_url) {
            return {
              title: data.author_name ? `${data.author_name} 的推文` : 'Twitter',
              image: data.thumbnail_url
            };
          }
          twitterTitle = data.author_name ? `${data.author_name} 的推文` : 'Twitter';
        }
      } catch (e) { }

      const match = url.match(/(?:twitter\.com|x\.com)\/([^\/\?]+)/);
      if (match && !['i', 'home', 'explore', 'notifications', 'messages'].includes(match[1])) {
        const username = match[1];
        if (!twitterTitle) twitterTitle = `@${username} 的推文`;
        twitterImageFallback = `https://unavatar.io/twitter/${username}`;
      }
    }
    ...
    if (url.includes('youtube.com') || url.includes('youtu.be')) {
      ...
      if (videoId) {
        ...
        return { title: '', image: `https://img.youtube.com/vi/${videoId}/mqdefault.jpg` };
      }
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (title || image) {
          return { 
            title: title ? title.trim() : (twitterTitle || ''), 
            image: image.startsWith('http') ? image : (image ? new URL(image, url).href : '')
          };
        }
      }
    } catch (e) { }

    return { title: twitterTitle || '', image: twitterImageFallback || '' };
  }
  ...
  window.handleClipboardFromNative = function(content) {
    ...
    if (match) {
      ...
      // 改为直接打开全功能捕获弹窗 (类似桌面端 capture.html)
      openCaptureModal(url);
    }
  };
